dimdesc.MCA <- function(resmca,ncp=3,proba=0.05) {
   ...
   for(i in 1:ncp) {
      if(classe %in% c('MCA','speMCA')) temp <- condes(data.frame(resmca$ind$coord[,i],X),1,0.05) # new
      if(classe == 'csMCA') temp <- condes(data.frame(resmca$ind$coord[,i],X[resmca$call$subcloud,]),1,0.05) # new
      res[[i]] <- temp
      }
